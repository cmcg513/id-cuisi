...
def upsample(recipes,t_labels):
	...
	new_rows = []
	for cuisine in t_labels:
		k = max_value - counts[cuisine]
		if k == 0:
			continue
		elif k < 0:
			raise ValueError('Something went wrong in getting the max!')
		filtered = recipes[recipes['cuisine'] == cuisine]
		k = int(k *(1.0/10.0))#7,10,5
		if k > len(filtered):
			rand = np.random.randint(0,999999)
			# print(rand)
			new_rows.append(filtered.sample(k,replace=True,random_state=rand))
		else:
			rand = np.random.randint(0,999999)
			# print(rand)
			new_rows.append(filtered.sample(k,random_state=rand))

	for entry in new_rows:
		recipes = recipes.append(entry)
	return recipes

# def downsample(recipes,t_labels):
# 	new_rows = None
# 	for cuisine in t_labels:
# 		k = 3000
# 		filtered = recipes[recipes['cuisine'] == cuisine]
# 		if k > len(filtered):
# 			if new_rows is None:
# 				new_rows = filtered.sample(k,replace=True)
# 			else:
# 				new_rows = new_rows.append(filtered.sample(k,replace=True))
# 		else:
# 			if new_rows is None:
# 				new_rows = filtered.sample(k)
# 			else:
# 				new_rows = new_rows.append(filtered.sample(k))
# 	# print(new_rows)
# 	return new_rows
...
def transform_and_fit(recipes):
	...
	recipes = upsample(recipes,t_labels)
	# recipes = downsample(recipes,t_labels)
...
def get_model(model_num,seed):
	...
	if model_num == 0:
		...
		params['clf__class_weight'] = [None]#,'balanced']		params['vect__tokenizer'] = [None]#[simple_tokenizer,None]
